...
(function configureEvergage() {
    try {
       Evergage.configure({
           account: "sparcgroupllc",
           dataset: "prodaero",
           cdnUrl: "https://cdn.evergage.com",
           trackerUrl: "https://sparcgroupllc.us-7.evergage.com",
           siteConfigVersion: "1"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["customerId","emailAddress","phone","profileId","sfmcContactKey","subscriberId"],"lastModified":1676472685060,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"secureCookie":true,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
//Cookie is created in optIn state by OneTrust, check upon beacon load cookie state if user has changed state
const checkCookie = () => { // Function to check cookie, called by the init function
    let cookieExist = document.cookie.split('; ').find((row) => row.startsWith('OptanonConsent')).split("group")[1].includes('4%3A1'); // Cookie dropped by oneTrust
    if (cookieExist) { // If cookie has optIn, handle based on preference documented in the cookie
        const trueConsent = [{
            provider: "OneTrust",
            purpose: SalesforceInteractions.mcis.ConsentPurpose.Personalization,
            status: SalesforceInteractions.ConsentStatus.OptIn // OptIn Passed
        }];
        return trueConsent;
    } else { // Cookie doesn't exist yet or doesn't have optIn
        document.dispatchEvent(new CustomEvent(SalesforceInteractions.CustomEvents.OnClearCookie)); // Delete SFIS cookie, in case it was created
        const falseConsent = [{
            provider: "OneTrust",
            purpose: SalesforceInteractions.mcis.ConsentPurpose.Personalization,
            status: SalesforceInteractions.ConsentStatus.OptOut // OptOut Passed
        }];
        return falseConsent;
    }
};
...
SalesforceInteractions.init({    // Initializes the Interaction Studio Web SDK
    cookieDomain: "aeropostale.com",   // Drops 1st party cookie
    consents: checkCookie() // Check CookieConsent status
}).then(() => {
    const sitemapConfig = {
        global: {
            onActionEvent: (actionEvent) => {
                if (true) { // User Attributes to be added in Phase 2
        ...
        pageTypes: [
            {
                name: "Homepage",
                isMatch: () => {
                    let re = new RegExp("^https?:\/\/(www\.)?aeropostale\.com\/?(\\?.*)?$"); // regex used to detect homepage, including any query parameters
            ...
            },
            {
                ...
                isMatch: () => new Promise((resolve, reject) => {
                    ...
                    return SalesforceInteractions.DisplayUtils.pageElementLoaded(".search-results-container", "html").then(() => {
                        clearTimeout(isMatchCat);
                        resolve(!/\/search/.test(window.location.pathname));
                    });
                }),
                interaction: {
                    name: "Category Page",
            ...
            },
            {
                ...
                isMatch: () => new Promise((resolve, reject) => {
                    ...
                    return SalesforceInteractions.DisplayUtils.pageElementLoaded("#pdpMain", "html").then(() => {
            ...
            {
                name: "Shipping Page",
                isMatch: () => /\/shipping/.test(window.location.href),
                interaction: {
                    name: "Shipping Page"
                }
            },
            {
                name: "Billing Page",
                isMatch: () => /\/billing/.test(window.location.href),
                interaction: {
                    name: "Billing Page"
                }
            },
            {
                ...
                isMatch: () => /\/placeorder/.test(window.location.href),
                interaction: {
                    name: "Place Order Page"
                }
            },
            ...
            {
                name: "Contact Us",
                isMatch: () => /\/contactus/.test(window.location.href),
                interaction: {
                    name: "Contact Us",
                }
            }
